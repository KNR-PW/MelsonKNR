function [Motion] = InverseKinematicsLF(Motion, GoalElem, q0,SolverOptions)
...
%     disp("left")
    ...
    if Motion.enable_aug
        rot_LF = Mat.RPY(Motion.X_Planned(19:21,GoalElem));
        
        X_goal  = [Motion.X_PlannedInLF(:,GoalElem);
                   Motion.X_aug(:,GoalElem)];
        X_start = [KinFastLF.eval_r(q0);
                   KinFastLF.eval_r_aug(q0, rot_LF)];
    else
        X_goal  = Motion.X_PlannedInLF(:,GoalElem);
        X_start = KinFastLF.eval_r(q0);
    end
    ...
    GeometricJacobian = KinFastLF.eval_J(q0);                          % Geometric Jacobian
    
    if Motion.enable_aug
        AugJacobian = KinFastLF.eval_J_aug(q0, rot_LF);          % Augmented Jacobian part
    else
        AugJacobian = [];
    end
...
function Torque = CalculateTorques(Motion, GoalElem, q)
    Jacobian = KinFastLF.eval_J(q);
    Div = [ones(12,19)*1000; ones(3,19)];
    Jacobian = Jacobian./Div;
    
    Force = Motion.ExtForcesInLF(:,GoalElem);
