...
class Humanoid:
    def __init__(self,                   # po utworzeniu obiektu dobrze jest postawic melsona funkcja wstawanie
                 portTTL='COM5',          # port, który obsluguje AX
                 portRS485='COM8'):           # port, który obsluguje RX
        ...
        self.poz_start = [0,2048, 512, 345, 740, 617, 512, 2048, 512, 679, 284, 407, 512, 497, 578, 230, 527, 446, 794, 477]
        ...
        self.slope_CW = [0, 32, 16, 16, 16, 16, 16, 32, 16, 16, 16, 16, 16, 32, 32, 32, 32, 32, 32, 64]
        self.slope_CCW = [0, 32, 16, 16, 16, 16, 16, 32, 16, 16, 16, 16, 16, 32, 32, 32, 32, 32, 32, 64]
    ...
    def send_pos1(self,poz):
        try:
            print("send pos try start")
            for i in range(1, 13):
            ...
            print("send pos try finish")
        except:
            print('blad z portami')
            self.res_port()
            self.send_pos(poz=poz)
        return

    def send_pos2(self, poz):
        #print("send pos try start")
        for i in range(1, 13):
            #print("Servo: " + str(i))
            self.packetHandler.write2ByteTxOnly(self.port_RX, i, self.ADR_POZ, poz[i])
    ...
    def send_pos(self, poz):
        #print("send pos try start")
        self.sync_write_2(self.ADR_POZ,poz)
    ...
    def setSlope(self,noga):
        #self.res_port()
        #print("send slope start")
        if(noga == 'B'):
            SlopeCW = [0,32, 2, 2, 2, 2, 2, 32, 2, 2, 2, 2, 2, 32, 32, 32, 32, 32, 32, 64]
            SlopeCCW = [0,32, 2, 2, 2, 2, 2, 32, 2, 2, 2, 2, 2, 32, 32, 32, 32, 32, 32, 64]
            #SlopeCW = [0, 32, 16, 16, 16, 16, 16, 32, 16, 16, 16, 16, 16, 32, 32, 32, 32, 32, 32, 64]
            #SlopeCCW = [0, 32, 16, 16, 16, 16, 16, 32, 16, 16, 16, 16, 16, 32, 32, 32, 32, 32, 32, 64]
            #SlopeCW = [0, 32, 16, 16, 16, 16, 16, 32, 16, 16, 16, 16, 16, 32, 32, 32, 32, 32, 32, 64]
            #SlopeCCW = [0, 32, 16, 16, 16, 16, 16, 32, 16, 16, 16, 16, 16, 32, 32, 32, 32, 32, 32, 64]

            self.sync_write_1(self.ADR_SLOPE_CW,SlopeCW)
            self.sync_write_1(self.ADR_SLOPE_CCW,SlopeCCW)

        if(noga == 'R'):
            SlopeCW = [0,32, 2, 2, 2, 2, 2, 32, 7, 7, 7, 7, 7, 32, 32, 32, 32, 32, 32, 64]
            SlopeCCW = [0,32, 2, 2, 2, 2, 2, 32, 7, 7, 7, 7, 7, 32, 32, 32, 32, 32, 32, 64]
            #SlopeCW = [0, 32, 4, 4, 4, 4, 4, 32, 31, 31, 31, 31, 31, 32, 32, 32, 32, 32, 32, 64]
            #SlopeCCW = [0, 32, 4, 4, 4, 4, 4, 32, 31, 31, 31, 31, 31, 32, 32, 32, 32, 32, 32, 64]
            SlopeCW = [0, 32, 4, 4, 4, 4, 4, 32, 31, 31, 31, 31, 31, 32, 32, 32, 32, 32, 32, 64]
            SlopeCCW = [0, 32, 4, 4, 4, 4, 4, 32, 31, 31, 31, 31, 31, 32, 32, 32, 32, 32, 32, 64]
        ...
        if(noga == 'L'):
            ...
            #SlopeCW = [0, 31, 31, 31, 31, 31, 31, 32, 4, 4, 4, 4, 4, 32, 32, 32, 32, 32, 32, 64]
            #SlopeCCW = [0, 31, 31, 31, 31, 31, 31, 32, 4, 4, 4, 4, 4, 32, 32, 32, 32, 32, 32, 64]
            SlopeCW = [0, 31, 31, 31, 31, 31, 31, 32, 4, 4, 4, 4, 4, 32, 32, 32, 32, 32, 32, 64]
            SlopeCCW = [0, 31, 31, 31, 31, 31, 31, 32, 4, 4, 4, 4, 4, 32, 32, 32, 32, 32, 32, 64]
