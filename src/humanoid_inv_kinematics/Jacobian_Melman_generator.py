...
def Jacobian_Melman(q,trajectory_vector):
    ...
    T = np.array([[np.ones((12, 12)), np.zeros((12, 3))],
                  [np.zeros((3, 12)), E]])
    ...
    d1_LF = R_RF_W @ (-r_CHW_RT + r_CHW_LT) + R_RF_LT @ r_LT_LS + R_RF_LS @ r_LS_LF
    u1_LF = - R_RF_RT @ kin.rotY(-RTy) @ kin.rotX(-RTx) @ Uz
    Jv1_LF = skew_matrix(u1_LF) @ d1_LF
    d1_RH = R_RF_W @ (-r_CHW_RT) + R_RF_CHW @ r_CHW_RA + R_RF_RA @ r_RA_RFA + R_RF_RFA @ r_RFA_RH
    u1_RH = u1_LF
